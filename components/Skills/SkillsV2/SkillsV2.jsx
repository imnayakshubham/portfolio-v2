    ...
    return (
        <motion.div className='w-full p-5' ref={skillscontainer} style={{ opacity }}>
            <div className='flex gap-5 flex-col justify-center align-middle'>
                ...
                <div className="w-full grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 md:grid-cols-3 gap-6 sm:px-0 md:px-0 items-center">
                    {techStack.map(({ id, src, title, style }) => (
                        <div className={`${currentTheme === "dark" ? "dark__shadow" : "light__shadow"} p-4 md:p-6 cursor-pointer border border-gray-200 transition duration-300 ease-in-out hover:border-[#00ffaa] hover:shadow-lg hover:border-1`} key={id}>
